...
const Addexpenses = () => {
  const { budgets, showAddExpense, setshowAddExpense, dispatch2 } =
  ...
  async function handleClick2() {
    ...
    await setshowAddExpense("hidden");
    amountRef.current.value = "";
    descRef.current.value = "";
  }
  return (
    <div
      className={`${showAddExpense} card absolute top-10 left-1/2 -translate-x-1/2 p-3 border-black border-2 bg-white h-fit w-10/12`}
    >
      <div>
        <div className="flex justify-between items-center mb-5">
          <h1 className="font-bold text-2xl">New Expense</h1>
          <button
            onClick={() => setshowAddExpense("hidden")}
